...
class ScatterObjects(bpy.types.Operator):
    ...
    def invoke(self, context, event):
        ...
        if self.target_object is None or len(self.objects_to_scatter) == 0:
            self.report(
                {'ERROR'}, "Select objects to scatter and a target object")
            return {'CANCELLED'}
    ...
    def enable_draw_callback(self):
        self._draw_callback_view = bpy.types.SpaceView3D.draw_handler_add(
            self.draw_view, (), 'WINDOW', 'POST_VIEW')

    def disable_draw_callback(self):
        bpy.types.SpaceView3D.draw_handler_remove(
            self._draw_callback_view, 'WINDOW')
...
class TargetCacheEntry:
    ...
    def get_batch(self, settings):
        ...
        if self.gpu_batch is None:
            self.gpu_batch = create_batch_for_matrices(
                self.get_matrices(settings), self.base_scale)
...
def make_duplicator(target_collection, source_object, matrices):
    ...
    duplicator = triangle_object_from_matrices(
        source_object.name + " Duplicator", matrices, triangle_scale)
...
class StrokeTarget(TargetProvider):
    ...
    def draw(self):
        ...
        draw_line_strip_batch(self.batch, color=(
            1.0, 0.4, 0.1, 1.0), thickness=5)
...
def scatter_around_stroke(stroke_points, bvhtree, settings):
    ...
    for point, local_seed in iter_points_on_stroke_with_seed(stroke_points, settings.density, settings.seed):
        matrix = scatter_from_source_point(
            bvhtree, point, local_seed, settings)
...
def scatter_from_source_point(bvhtree, point, seed, settings):
    ...
    up_direction = normal if settings.use_normal_rotation else Vector(
        (0, 0, 1))
    ...
    # Rotation
    z_rotation = Euler((0, 0, random_uniform(
        sub_seed(seed, 3), 0, 2 * math.pi))).to_matrix()
    up_rotation = up_direction.to_track_quat('Z', 'X').to_matrix()
    local_rotation = random_euler(
        sub_seed(seed, 3), settings.rotation).to_matrix()
...
def create_batch_for_matrices(matrices, base_scale):
    ...
    for matrix in matrices:
        ...
        indices.extend(tuple(index + offset for index in element)
